...
We first verify the $\mathsc{D3SAT}$ problem through the same witness function as with $\mathsc{SAT}$, which we know is a polynomial one. Thus, $\mathsc{D3SAT}$ is in $\mathsf{NP}$.

To prove that the $\mathsc{D3SAT}$ problem is $\mathsf{NP}$-hard, we will reduce from $\mathsc{SAT}$ to $\mathsc{D3SAT}$. Given an instance of $\mathsc{SAT}$, we can construct an equivalent instance of $\mathsc{D3SAT}$ by transforming each clause into equivalent clauses with three distinct variables. We separate the processing of these clauses into four different cases:
...
It is direct to see that the first and second cases are equivalent to the original $\mathsc{SAT}$ instance. The third and fourth cases are equivalent to the original $\mathsc{SAT}$ instance because the new variables $x_2$ and $x_3$ are not present in any other clause, and the new clauses are true if and only if the original clause is true. 

The transformation is linear in the size of the problem, the fourth case being the worst, where for 1 original clause we create 4. Thus, since the resulting instance of $\mathsc{D3SAT}$ is satisfiable if and only if the original $\mathsc{SAT}$ instance is satisfiable, $\mathsc{D3SAT}$ is $\mathsf{NP}$-hard.
# 3
